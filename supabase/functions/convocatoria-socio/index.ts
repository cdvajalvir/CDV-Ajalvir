...
serve(async (req) => {
  ...
  try {
    ...
    // 2. Crear cliente de Supabase respetando el contexto del usuario autenticado (para saber quién es)
    ...
    // 3. Crear cliente con privilegios de Administrador (service_role) para saltar RLS en operaciones de lectura/escritura controlada
    ...
    if (action === 'cargar_activa_socio') {
      ...
      if (!convocatorias || convocatorias.length === 0) {
        return new Response(JSON.stringify({ data: null, estaApuntado: false }), {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        })
      }
    ...
    // ==========================================
    // ACCIÓN 2: TOGGLE ASISTENCIA (APUNTARSE / DESAPUNTARSE)
    // ==========================================
    if (action === 'toggle_asistencia') {
      ...
      // Consultar la convocatoria actual
      ...
      // Actualizar la lista de usuarios en la base de datos
